...
def get_array_excel(path):
    data_arr=np.array(pd.read_csv(path,encoding='GBK'))
    return data_arr
#获取数据范围
...
def main_IDW(path_point_xls,cell_len,n,p):
    data_arr = get_array_excel(path_point_xls)
    data_range = [623935,629935,3959412,3965412]
...
if __name__ =="__main__":
    ...
    pd.DataFrame(data_IDW).to_csv('data/IDW_含气量.csv',index=0,header=0)
