...
class Gas_prediction():
    ...
    def get_gas_prediction(self,P,k_g,Z,P_wf,K,r_e):
        '''
        气体产能计算
        :param P:压力
        :param k_g:气相渗透率
        ...
        :return:q_g，日产气量，m^3
        '''
        q_g=774.6*K*k_g*self.h*(P**2-P_wf**2)/(self.T*self.mu_g*Z*( np.log(r_e/self.r_w)-0.75+self.s ))
        return q_g

    def get_water_prediction(self,P,k_w,P_wf,K,S_w,r_e):
        '''
        产水量计算
        :param P:
        :param k_w:
        ...
        :return: q_w，日产水量，m^3
        '''
        q_w=0.543*k_w*S_w*K*self.h*(P-P_wf)/(self.B_W*self.mu_w*( np.log(r_e/self.r_w)-0.75+self.s ))
        return q_w
