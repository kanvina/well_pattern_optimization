...
class GA_model_grid():
    ...
    def compute_grids(self,grids_pop):
        ...
        for DNA_line_value2 in grids_pop:
            ...
            x_zoom=self.translate_value_2(DNA_line_value2[0])
            y_zoom=self.translate_value_2(DNA_line_value2[1])
            theta=self.translate_value_2(DNA_line_value2[2])
            gamma=self.translate_value_2(DNA_line_value2[3])
            Delta_x=self.translate_value_2(DNA_line_value2[4])
            Delta_y=self.translate_value_2(DNA_line_value2[5])
    ...
    def compute_grids_save(self,save_grids_pop):
        ...
        for DNA_line_value2 in save_grids_pop:
    ...
    def select(self,pop_result_list,grids_pop):  # 根据概率选择
        grids_pop_copy=copy.deepcopy(grids_pop)
        ...
        idx = np.random.choice(np.arange(len(drop_grids_pop_parent)), size=len(drop_grids_pop_parent), replace=True,p=(fitness / np.sum(fitness)))
    ...
    def crossover(self,drop_grids_pop_parent,save_grids_pop_parent):
        ...
        drop_grids_pop_parent_copy=drop_grids_pop_parent
        ...
        for n in range(len(drop_grids_pop_parent_copy)):
            if np.random.rand() < self.GA_model['cross_rate']:
                ...
                for i in range(len(drop_grids_pop_parent_copy[n])):
                    grid_child = []

                    grid_DNA_parent = drop_grids_pop_parent_copy[n][i][1]
                    ...
                    drop_grids_pop_parent_copy[n][i][1] = grid_child

        drop_grids_pop_parent_mutate=self.mutate(drop_grids_pop_parent_copy)
...
if __name__=="__main__":
    ...
    for i in range(GA.GA_model['num_iteration']):
        ...
        grids_pop=GA.crossover(drop_grids_pop_parent,save_grids_pop_parent)
